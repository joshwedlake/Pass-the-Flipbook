...
function lookup_parent_data($parent_seq_id){
	...
	if($parent_seq_id!='-1'){
		...
		return array($animations_record_row['frames_total'],$animations_record_row['flags'],$animations_record_row['tags']);
	}
	else return array(0,0,serialize(array()));
}
...
$date_created=date("c");
list($frames_total,$flags,$tags)=lookup_parent_data($_POST["parent_seq_id"]);
$pass=rand();

$query="INSERT INTO animations(pass,name,animator,parent_seq_id,date_created,date_last_viewed,frames_this,frames_total,thumbs_up,thumbs_down,thumbs_score,viewed,flags,tags,saved) VALUES('".
	...
	mysql_real_escape_string($tags)."','1')";
mysql_query($query) or die("1,0,0,");

// add the animation to its tag's list
